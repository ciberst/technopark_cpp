...
namespace {
void ambig_call() {
  std::cout << "ambig_call 1" << std::endl;
}
}  // namespace

void ambig_call() {
  std::cout << "ambig_call 2" << std::endl;
}

namespace N1 {
class A {
 public:
  void print() { std::cout << "N1::A" << std::endl; }

  static void static_print() { std::cout << "N1::A" << std::endl; }
};
}  // namespace N1
...
namespace www::example::com {  // since C++17
    void print2() {
        std::cout << "www::example::com::print2()" << std::endl;
    }
}
...
struct A {
  static void print() { std::cout << "local::A::print()" << std::endl; }
};
...
void foo(float a) {
  std::cout << "global_names_search::inner_names_search::foo(float)"
            << std::endl;
}
...
// after print => not found
void bar(float a) {
  std::cout << "global_names_search::inner_names_search::bar(float)"
            << std::endl;
}
}  // namespace inner_names_search
}  // namespace global_names_search
...
void before_print() {
  std::cout << "using_namespace_example_outer::with_using::before_print()"
            << std::endl;
}

namespace inner {
void before_print() {
  std::cout << "using_namepace_example_outer::with_using::inner::before_print()"
            << std::endl;
}
}  // namespace inner
...
namespace inner {
void after_print() {
  cout << "using_namepace_example_outer::with_using::inner::after_print()"
       << endl;
}
}  // namespace inner
...
inner::SumObject operator-(inner::SumObject const& a,
                           inner::SumObject const& b) {
...
int main(int argc, char* argv[]) {
  ...
  // operator from namespace - Koenig's search (ADL, Argument-dependent param1
  // lookup)
  ...
  {
    int k = 0;
    std::cout << "Scope defines new namespace. For example, k = " << k
              << std::endl;
    {
      int k = 42;
      std::cout << "Now we defined new k in inner scope => in inner namespace, "
                   "so now k = "
                << k << std::endl;
    }
    std::cout << "Old k did not change: k = " << k << std::endl;
    {
      using std::cout;
      using std::endl;
      cout << "At least, prefer explicit namespace prefix or use aliases for "
              "long ones"
           << endl;
    }
    using namespace std;
    cout << "And don't use 'using namespace std' etc in header files!!!"
         << endl;
  }
  return 0;
}
